...
def viccall(soildata,rank,order,number_of_days):								# Modify when needed
            ...
            if (sl[2]!=0):														#Avoid add more row if the last row is blank
                ...
                for k in range(10):
                    if (VIC_vars[k]>0):
                        ...
                        count_no+=1
                if (no_of_row ==53):											#3-layer soil - not test yet, if there are any errors, modify Lines 71
    ...
    np.savetxt(str(order)+".txt",discharge,fmt="%s")
    return discharge
...
for j in range(number_of_samples):
    ...
    for i in range(spinning_period,number_of_days):				# Only calculate NSE and TRMSE after the spinning period
        zmdl = (pow((modelleddischarge[j][i]+1),0.3)-1)/0.3
        zobs = (pow((gaudata[i]+1),0.3)-1)/0.3
        trmse+=pow(zmdl-zobs,2)
        numerator+=pow(modelleddischarge[j][i]-gaudata[i],2)
        meangau+=gaudata[i]
        summdl+=modelleddischarge[j][i]
        sumobs+=gaudata[i]
